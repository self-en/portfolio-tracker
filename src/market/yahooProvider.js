...
const { tolerant } = require("./tolerant");
 ...
 */
const pinoAdapter = {
  info: (...args) => logger.info({ yf: args.map(brief) }, "[yahoo] info"),
  warn: (...args) => logger.warn({ yf: args.map(brief) }, "[yahoo] warn"),
  error: (...args) => logger.error({ yf: args.map(brief) }, "[yahoo] error"),
  debug: (...args) => logger.debug({ yf: args.map(brief) }, "[yahoo] debug"),
  // `dir` è quello che si dimentica: la libreria lo usa per gli oggetti.
  dir: (...args) => logger.debug({ yf: args.map(brief) }, "[yahoo] dir"),
};
 ...
 */
function normalizeEvents(chart) {
  const ev = chart?.events;
  const out = { dividends: [], splits: [] };
...
function normalizeSearch(result) {
  ...
  return quotes
    .filter((q) => q && q.symbol && q.isYahooFinance !== false)
    .map((q) => ({
      symbol: q.symbol,
...
function normalizeUpcomingDividend(summary) {
...
class CircuitBreaker {
  constructor({ threshold = BREAKER_THRESHOLD, cooldownMs = BREAKER_COOLDOWN_MS } = {}) {
...
function createYahooProvider(cfg) {
  ...
  async function call(label, fn) {
    ...
    for (let attempt = 0; attempt <= BACKOFF_MS.length; attempt++) {
      try {
        ...
        logger.warn(
          { label, attempt: attempt + 1, waitMs: wait, err: String(err.message).slice(0, 200) },
  ...
  return {
    ...
    async getQuotes(symbols) {
      ...
      for (let i = 0; i < settled.length; i++) {
        ...
        if (r.status !== "fulfilled") {
          ...
          logger.warn(
            { symbol: list[i], err: String(r.reason?.message).slice(0, 200) },
            "[market] quotazione non recuperata"
          );
          continue;
        }
        const q = normalizeQuote(r.value);
        if (q) out.push(q);
      }
      return out;
    },
    ...
    async getHistory(symbol, from, to) {
      const chart = await call(`chart ${symbol}`, () =>
        yf.chart(symbol, {
          period1: from,
          period2: to,
          interval: "1d",
    ...
    async getCorporateActions(symbol, from, to) {
      const chart = await call(`chart-events ${symbol}`, () =>
        yf.chart(symbol, {
          period1: from,
          period2: to,
          interval: "1d",
    ...
    async getUpcomingDividend(symbol) {
      const summary = await call(`quoteSummary ${symbol}`, () =>
    ...
    async resolveSymbol(query) {
      const result = await call(`search ${query}`, () => yf.search(query));

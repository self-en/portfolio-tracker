...
const { Leadership, SCHEDULER_LOCK_KEY } = require("../db/leader");
...
function leaderOnly(name, fn) {
  return async () => {
    ...
    try {
      ...
      logger.error({ job: name, err: err.message }, "[scheduler] job fallito");
    }
  };
}
...
async function catchUp() {
  for (const [job, maxAge] of Object.entries(CATCHUP_MAX_AGE_MS)) {
    try {
      ...
      logger.warn({ job, err: err.message }, "[scheduler] catch-up fallito per questo job");
    }
  }
}

...
const { createFxProvider } = require("./fxProvider");
  ...
  try {
    while (queue.length > 0) {
      ...
      try {
        ...
        stats.failed += 1;
        stats.lastError = String(err?.message).slice(0, 300);
...
async function backfillInstrument(instrumentId, opts = {}) {
  ...
  try {
    ...
    await refreshLog.finish(logId, { ok: false, error: err.message });
    throw err;
  }
}
  ...
  try {
    ...
    for (const inst of list) {
      try {
        ...
        logger.warn(
          { instrumentId: inst.id, ticker: inst.ticker, err: String(err.message).slice(0, 200) },
  ...
  try {
    ...
    for (const inst of list) {
      try {
        ...
        logger.warn(
          { instrumentId: inst.id, err: String(err.message).slice(0, 200) },
...
async function reconcile() {
  try {
    ...
    logger.error({ err: err.message }, "[refresher] reconciler fallito (continuo)");
    return 0;
  }
}

...
const { D, d, ZERO, ONE, HUNDRED, toBase, isBlank } = require("./money");
 ...
 */
function sortLedger(txs) {
  return [...txs].sort((a, b) => {
    const c = cal.cmp(cal.normalizeDate(a.tradeDate ?? a.trade_date), cal.normalizeDate(b.tradeDate ?? b.trade_date));
    if (c !== 0) return c;
...
function normalizeTx(tx) {
  return {
    ...
    price: tx.price,
    grossAmount: tx.grossAmount ?? tx.gross_amount,
    fees: tx.fees,
    taxes: tx.taxes,
    accruedInterest: tx.accruedInterest ?? tx.accrued_interest,
    netAmount: tx.netAmount ?? tx.net_amount,
    tradeCcy: tx.tradeCcy ?? tx.trade_ccy,
    fxRate: tx.fxRate ?? tx.fx_rate,
    splitRatio: tx.splitRatio ?? tx.split_ratio,
  };
}
 ...
 */
function tradeGross(tx, instrument) {
...
function positionValue(quantity, price, instrument) {
  const q = d(quantity);
  const p = d(price);
  if (instrument && instrument.quoteConvention === "PCT_OF_NOMINAL") {
    return q.times(d(instrument.faceValue, 1)).times(p).div(HUNDRED);
 ...
 */
function resolveFx(tx, baseCcy, fxLookup, warnings) {
...
function emptyPosition(instrumentId) {
  return {
    instrumentId,
    quantity: ZERO,
 ...
 */
function buildPositions(txs, opts = {}) {
 ...
 */
function splitAdjustedQuantitySeries(txs, dates) {
  const ordered = sortLedger(txs.map(normalizeTx));
 ...
 */
function costSeries(txs, dates, opts = {}) {
  const baseCcy = (opts.baseCcy || "EUR").toUpperCase();
  const instruments = opts.instruments || new Map();
  const fxLookup = opts.fxLookup;
  const sink = [];
 ...
 */
function valuePosition(position, instrument, marketPrice, fxRate, opts = {}) {

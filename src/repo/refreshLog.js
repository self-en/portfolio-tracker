...
async function start(job, target = null) {
...
async function finish(id, { ok, error = null, rowCount = null }) {
  await query(
...
async function lastSuccess(job) {
  const { rows } = await query(
...
async function recent(job = null, limit = 50) {
  const { rows } = await query(
...
module.exports = { start, finish, lastRuns, lastSuccess, recent, prune };

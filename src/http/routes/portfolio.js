...
const valuation = require("../../domain/valuation");
const returns = require("../../domain/returns");
const cal = require("../../domain/calendar");
const { d } = require("../../domain/money");
const S = require("../serialize");
const { asyncHandler, validation } = require("../errors");
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (groups.length > MAX_SLICES) {
      ...
      const { ZERO } = require("../../domain/money");
      head.push({
        key: "__altro__",
...
router.get(
  ...
  query(
    schemas.portfolioQuery.extend({
      from: require("../validate").dateString().optional(),
      to: require("../validate").dateString().optional(),
      groupBy: require("../validate").z.enum(["month", "instrument"]).default("month"),
    })
  ),
  asyncHandler(async (req, res) => {
...
module.exports = { router, loadValuation, today };

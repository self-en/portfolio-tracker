...
const bonds = require("../../domain/bonds");
const { money } = require("../../domain/money");
const { asyncHandler, notFound, conflict, validation } = require("../errors");
const { z, body, query, params, parse, idParam, dateString } = require("../validate");
...
async function regenerateProjected(inst) {
  ...
  try {
    ...
    logger.error(
      { instrumentId: inst.id, err: err.message },
  ...
  asyncHandler(async (req, res) => {
    ...
    if (inst.assetClass === "BOND" && inst.maturityDate) {
      try {
        ...
        out.couponSchedule = [];
        out.warnings = [{ code: "schedule_error", message: err.message }];
      }
    }
...
module.exports = { router, regenerateProjected, withBondDetails };

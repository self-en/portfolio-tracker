...
const { migrate, knownVersions } = require("./db/migrate");
...
async function runMigrations() {
  ...
  for (let attempt = 0; attempt <= RETRY_DELAYS_MS.length; attempt++) {
    try {
      ...
      state.db.error = err.message;
      state.migrations.error = err.message;
      if (delay === undefined) {
...
async function start() {
  ...
  try {
    ...
    logger.error({ err: err?.message, stack: err?.stack }, "[boot] errore inatteso nel boot");
  }

  return state;
}

module.exports = { start, state, runMigrations, RETRY_DELAYS_MS };

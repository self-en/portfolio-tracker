...
/**
 * Cio' che i formattatori accettano: una STRINGA decimale (la forma su cui
 * viaggia il denaro, docs/decisions.md §1), oppure null/undefined per "assente".
 * I number sono ammessi per comodita' di chi passa un conteggio, mai per il denaro.
 */
...
// Type guard, non un semplice boolean: e' cio' che fa restringere il tipo dopo
// `if (isBlank(value)) return DASH;` in ogni formattatore.
const isBlank = (v: Formattable): v is null | undefined | "" =>
  v === null || v === undefined || v === "";

/**
 * Porta il valore in una forma che Intl accetta, SENZA aritmetica: una stringa
 * resta stringa (Intl la formatta senza passare da un float, che e' il punto).
 * null se non e' un decimale riconoscibile.
 */
function decimal(value: Formattable): number | Intl.StringNumericLiteral | null {
  if (isBlank(value)) return null;
  if (typeof value === "number") return Number.isFinite(value) ? value : null;
  const s = String(value).trim();
  if (!DECIMAL.test(s)) return null;
  // Intl non digerisce il "+" iniziale. Il cast e' lecito perche' la regex sopra
  // ha appena verificato che la stringa E' un letterale numerico: Intl.format()
  // accetta le stringhe proprio per non passare da un float.
  return (s.startsWith("+") ? s.slice(1) : s) as Intl.StringNumericLiteral;
}
...
export function money(value: Formattable, ccy = "EUR"): string {
...
export function num(value: Formattable, dp = 2): string {
  const d = decimal(value);
  if (d === null) return DASH;
 ...
 */
function shiftTwo(s: string): string {
...
export function pct(value: Formattable, dp = 2): string {
  const d = decimal(value);
  if (d === null) return DASH;
...
export function qty(value: Formattable): string {
  const d = decimal(value);
  if (d === null) return DASH;
 ...
 */
export function date(value: Formattable): string {
...
/** "2026-09" → "set 2026". */
export function monthLabel(value: Formattable): string {
  if (isBlank(value)) return DASH;
 ...
 */
export function time(value: Formattable): string {
  if (isBlank(value)) return DASH;
...
export function dateTime(value: Formattable): string {
  if (isBlank(value)) return DASH;
  const d = new Date(value);
  if (Number.isNaN(d.getTime())) return DASH;
  return dtf("datetime", {
...
export function signed(value: Formattable, dp = 2): string {
  const d = decimal(value);
  if (d === null) return DASH;
  return nf(`signed:${dp}`, {
...
export function signedMoney(value: Formattable, ccy = "EUR"): string {
  const d = decimal(value);
  if (d === null) return DASH;
...
export function signedPct(value: Formattable, dp = 2): string {
  const d = decimal(value);
  if (d === null) return DASH;
  const scaled = typeof d === "number" ? d * 100 : shiftTwo(d);
 ...
 */
export function signOf(value: Formattable): -1 | 0 | 1 | null {
  const d = decimal(value);
  if (d === null) return null;
  if (typeof d === "number") return Math.sign(d) as -1 | 0 | 1;
...
export function toneOf(value: Formattable): string {

import { money, signedMoney, toneOf, DASH } from "../format.js";
 ...
 */
export default function Money({ value, ccy = "EUR", withSign = false, tone = false, className }) {
...
export function Field({ label, hint, error, required = false, children, wide = false }) {
  return (
 ...
 */
export function DecimalInput({ value, onChange, placeholder, disabled, readOnly, id, ...rest }) {
  return (
    <input
      id={id}
      className="input num"
...
export function ReadOnlyValue({ children, title }) {
  return (
